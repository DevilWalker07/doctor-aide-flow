/**
 * Prompts dos cinco especialistas virtuais.
 *
 * Texto preservado do commit 64cbf60, que criou o recurso. Saiu do código nas
 * fases A e C do refactor, junto com o serviço Python e as edge functions —
 * e é trabalho clínico que não se reescreve de memória.
 *
 * Moveu de `src/lib/specialists/prompts.ts` para cá: no cliente, estes prompts
 * iam no bundle e podiam ser lidos por qualquer um. Prompt que define regra de
 * segurança clínica não vai para o navegador.
 *
 * O contrato de JSON no fim de COMMON_RULES é validado por
 * `ParecerEspecialistaSchema` com Zod — antes nada validava a saída.
 */

import type { EspecialistaId } from "../../shared/especialistas.js";
...
export const PROMPTS_ESPECIALISTAS: Record<EspecialistaId, string> = {
