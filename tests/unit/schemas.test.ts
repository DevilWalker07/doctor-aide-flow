...
describe("schemas de saída da IA", () => {
  ...
  it("PatientRow: quadroAtual ganha prefixo canônico e _raciocinio é descartado", () => {
    expect(PatientRowSchema.parse({ leito: "L1", quadroAtual: "paciente bem", _raciocinio: "x" })).not.toHaveProperty("_raciocinio");
    expect(PatientRowSchema.parse({ leito: "L1", quadroAtual: "paciente bem" }).quadroAtual).toBe("NÃO CLASSIFICADO — paciente bem");
    expect(PatientRowSchema.parse({ leito: "L1", quadroAtual: "estavel - ok" }).quadroAtual).toBe("ESTÁVEL — ok");
    expect(PatientRowSchema.parse({ leito: "L1", quadroAtual: "CRÍTICO — MOF" }).quadroAtual).toBe("CRÍTICO — MOF");
    expect(PatientRowSchema.parse({ leito: "L1" }).quadroAtual).toBe("NÃO CLASSIFICADO — NÃO REFERIDO");
  });

  it("PatientRow: campos ausentes recebem fallbacks e di é limitado", () => {
    const row = PatientRowSchema.parse({ leito: "L1", di: "999", alertasPendencias: null, atb: 12 });
  ...
  it("AlertaCritico: prioridade desconhecida vira !! URGENTE (fail-safe)", () => {
    expect(AlertaCriticoSchema.parse({ prioridade: "alta", acao: "x" }).prioridade).toBe("!! URGENTE");
    expect(AlertaCriticoSchema.parse({ prioridade: "hoje mesmo", acao: "x" }).prioridade).toBe("! HOJE");
    expect(AlertaCriticoSchema.parse({ prioridade: "social", acao: "x" }).prioridade).toBe("PENDÊNCIA SOCIAL");
  ...
  it("PassagemPlantaoBatch: pacientes inválidos rejeitam o lote; alertas inválidos viram []", () => {
    expect(() => PassagemPlantaoBatchSchema.parse({ pacientes: "nao", alertasCriticos: [] })).toThrow();
    expect(PassagemPlantaoBatchSchema.parse({ pacientes: [], alertasCriticos: "nao" }).alertasCriticos).toEqual([]);
  });

  it("DocumentExtraction: coerção e limites", () => {
    const d = DocumentExtractionSchema.parse({ idade: "88", sexo: "masculino", data_admissao: "12/09/2026", lista_de_problemas: "PAC" });
  ...
  it("ClinicalPatient: id espelha leito e sexo normalizado", () => {
    const out = ClinicalExtractionOutputSchema.parse({ patients: [{ leito: "L07", sexo: "f", alertas: "x" }] });
