...
function AmbientePage() {
  ...
  const [subId, setSubId] = useState<string>(() =>
    ambiente.subs.some((s) => s.id === sub) ? sub! : ambiente.subs[0].id,
  );
  const subAtual = ambiente.subs.find((s) => s.id === subId) ?? ambiente.subs[0];

  const iniciar = () =>
    nav({ to: "/iniciar-plantao", search: { ambiente: ambiente.id, sub: subAtual.id } });

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        <div
          className={`absolute -top-32 right-0 h-[420px] w-[420px] rounded-full ${ambiente.accent.bg} blur-[140px]`}
        />
      </div>
              ...
              return (
                <button
                  ...
                  className={`text-left rounded-2xl border p-4 transition-all focus-visible:outline-none focus-visible:ring-2 ${ambiente.accent.ring} ${
                    ativo
                      ? `${ambiente.accent.border} ${ambiente.accent.bg} ring-1 ${ambiente.accent.ring}`
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="flex items-center gap-2 text-sm font-black uppercase tracking-wide">
                      {s.icon ? (
                        <s.icon
                          className={`h-4 w-4 ${ativo ? ambiente.accent.text : "text-slate-500"}`}
                        />
                      ) : null}
                      {s.label}
                    </span>
        ...
        <div className="rounded-[1.75rem] border border-slate-800 bg-slate-900/60 p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-400">
              Na próxima tela você informa a unidade e a data. O tipo de evolução e os agentes já
              ficam configurados.
            </p>
          </div>
          <button
