...
function PrescricaoPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setPaciente(p);
        setMedicacoes(
          (p.medications || []).map((m: string) => ({ id: Math.random().toString(), text: m })),
        );
  ...
  const handleSave = async () => {
    ...
    const prescricaoContent = {
      ...
      sintomaticos,
      profilaxias,
      hidratacao: hidratacao.active
        ? `${hidratacao.tipo} ${hidratacao.volume}ml (${hidratacao.velocidade}ml/h)`
        : "Nenhuma",
      exames,
      pendencias,
      ...(tipoUnidade === "uti"
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {CARE_CHIPS.map((opt) => (
              <Chip
                key={opt}
                label={opt}
                selected={cuidados.includes(opt)}
                onClick={() =>
                  setCuidados((prev) =>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {VITALS_FREQ.map((opt) => (
              <Chip
                key={opt}
                label={opt}
                selected={vitalsFreq === opt}
                onClick={() => setVitalsFreq(opt)}
              />
            ))}
          </div>
          <div className="flex flex-wrap gap-2">
            {VITALS_PARAMS.map((opt) => (
              <Chip
                key={opt}
                label={opt}
                selected={vitalsParams.includes(opt)}
                onClick={() =>
                  setVitalsParams((prev) =>
                    prev.includes(opt) ? prev.filter((i) => i !== opt) : [...prev, opt],
                  )
                }
              />
          ...
          <div className="space-y-3">
            {medicacoes.map((m, idx) => (
              <div key={m.id} className="flex gap-2">
                ...
                />
                <button
                  onClick={() => setMedicacoes(medicacoes.filter((item) => item.id !== m.id))}
        ...
        <Section title="5. ANTIBIÓTICOS" icon={<FlaskConical className="h-4 w-4" />}>
          <div className="space-y-3">
            {antibioticos.map((a, idx) => (
              <div
                key={idx}
                className="flex gap-2 items-center p-4 bg-ai/5 border border-ai/20 rounded-2xl"
              >
                <div className="flex-1">
                  <p className="text-xs font-black text-ai uppercase tracking-tight">
                    {a.nome} {a.dose} {a.via} {a.frequencia}
                  </p>
                </div>
                ...
                <button
                  onClick={() => setAntibioticos(antibioticos.filter((_, i) => i !== idx))}
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {SYMPTOMATIC_CHIPS.map((opt) => (
              <Chip
                key={opt}
                label={opt}
                selected={sintomaticos.includes(opt)}
                onClick={() =>
                  setSintomaticos((prev) =>
                    prev.includes(opt) ? prev.filter((i) => i !== opt) : [...prev, opt],
                  )
                }
              />
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {PROPHYLAXIS_CHIPS.map((opt) => (
              <Chip
                key={opt}
                label={opt}
                selected={profilaxias.includes(opt)}
                onClick={() =>
                  setProfilaxias((prev) =>
                    prev.includes(opt) ? prev.filter((i) => i !== opt) : [...prev, opt],
                  )
        ...
        <Section title="8. HIDRATAÇÃO" icon={<Activity className="h-4 w-4" />}>
          ...
          {hidratacao.active && (
            <div className="grid md:grid-cols-3 gap-4 animate-in fade-in slide-in-from-top-2">
              <select
                value={hidratacao.tipo}
                onChange={(e) => setHidratacao({ ...hidratacao, tipo: e.target.value })}
                className={inputCls}
              >
              ...
              <ControlledInput
                value={hidratacao.volume}
                onValueChange={(val) => setHidratacao({ ...hidratacao, volume: val })}
                placeholder="VOLUME (ML)"
              />
              <ControlledInput
                value={hidratacao.velocidade}
                onValueChange={(val) => setHidratacao({ ...hidratacao, velocidade: val })}
                placeholder="VELOCIDADE (ML/H)"
              />
            </div>
          )}
        </Section>
            ...
            {exames.map((e, idx) => (
              <div key={idx} className="flex gap-2">
                <ControlledInput
                  value={e}
                  onValueChange={(val) =>
                    setExames(exames.map((item, i) => (i === idx ? val : item)))
                  }
                  uppercase
                />
                <button
                  onClick={() => setExames(exames.filter((_, i) => i !== idx))}
                  className="p-4 rounded-xl bg-destructive/10 text-destructive"
                >
                  <Trash2 className="h-4 w-4" />
                </button>
              ...
              <div className="space-y-3">
                {dvas.map((d, idx) => (
                  <div key={d.id} className="flex gap-2">
                    ...
                    />
                    <button
                      onClick={() => setDvas(dvas.filter((item) => item.id !== d.id))}
                      className="p-4 rounded-xl bg-destructive/10 text-destructive"
                    >
                      <Trash2 className="h-4 w-4" />
                    </button>
              ...
              <div className="space-y-3">
                {sedacao.map((s, idx) => (
                  <div key={s.id} className="flex gap-2">
                    ...
                    />
                    <button
                      onClick={() => setSedacao(sedacao.filter((item) => item.id !== s.id))}
                      className="p-4 rounded-xl bg-destructive/10 text-destructive"
                    >
                      <Trash2 className="h-4 w-4" />
                    </button>
