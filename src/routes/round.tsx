      ...
      <aside className="w-80 bg-white border-r border-[#DCE4F0] flex flex-col shrink-0 overflow-hidden no-print">
        <div className="px-6 pt-6 pb-4 border-b border-[#DCE4F0] bg-[#F8FAFC]">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#8A9BB8]">
              CENSO ATUAL
            </h2>
            ...
            patients.map((p) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <ChevronRight
                    className={`h-3 w-3 transition-transform ${selectedId === p.id ? "text-[#2563EB] translate-x-1" : "text-[#CBD5E1]"}`}
                  />
                </div>
                <p className="text-[11px] font-bold text-[#0B1F44] truncate mb-1 uppercase">
                  {p.nome.split(" ").slice(0, 2).join(" ")}
                </p>
                <p className="text-[9px] text-[#8A9BB8] font-bold truncate uppercase">
                  {p.diagnosticos}
                </p>
                {p.alertas.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {p.alertas.slice(0, 2).map((t) => (
                      <span
                        key={t}
                        className="text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-md bg-[#FEF2F2] text-[#EF4444] border border-[#FCA5A5]/20"
                      >
                        {t}
                      </span>
                    ))}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-[#DCE4F0] px-8 flex items-center justify-between shrink-0 no-print shadow-sm z-10">
          <div className="flex items-center gap-4">
            <Link
              to="/dashboard"
              className="h-10 w-10 rounded-xl flex items-center justify-center text-[#64748B] hover:bg-[#F1F5F9] transition-all"
            >
            ...
            <div className="h-full flex flex-col items-center justify-center max-w-2xl mx-auto text-center space-y-6">
              ...
              <div>
                <h2 className="text-2xl font-black text-[#0F172A] mb-2 uppercase tracking-tight">
                  Importe as evoluções de ontem
                </h2>
                <p className="text-sm text-[#8A9BB8] leading-relaxed font-bold">
                  PARA GERAR O MAPA DE PASSAGEM INTELIGENTE, PRECISAMOS PROCESSAR AS EVOLUÇÕES DO
                  DIA ANTERIOR. VOCÊ PODE COLAR O TEXTO OU ENVIAR ARQUIVOS.
                </p>
              </div>
              <button
                onClick={() => setShowImport(true)}
            ...
            <>
                ...
                {patients
                  .map((p) =>
                    p.alertas.map((a) => (
                      <div
                        key={`${p.id}-${a}`}
                        className="flex items-center gap-3 bg-white border border-[#FCA5A5] rounded-2xl p-4 shrink-0 shadow-sm shadow-red-100"
                      >
                        ...
                        <div>
                          <p className="text-[10px] font-black text-[#EF4444] uppercase tracking-widest">
                            {a}
                          </p>
                          <p className="text-[9px] font-bold text-[#8A9BB8] uppercase">
                            {p.nome.split(" ")[0]}
                          </p>
                        </div>
                      </div>
              ...
              <div className="flex items-start justify-between mb-8">
                <div>
                  <div className="flex items-center gap-4 mb-2">
                    ...
                    <span className="text-2xl font-black text-[#0B1F44] uppercase tracking-tight">
                      {patient.nome}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  <Badge variant="ai" size="xl">
                    MOTOR LUAN — IA SIMULADA
                  </Badge>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-8">
                <div className="space-y-8">
                  <Card
                    title="RESUMO DO QUADRO ATUAL"
                    icon={<Sparkles className="h-5 w-5 text-[#2563EB]" />}
                    accent
                  >
                    <p className="text-[13px] leading-[1.8] text-[#0B1F44] font-bold uppercase">
                      {patient.quadro}
                    </p>
                  </Card>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <Card
                      title="ANTIBIÓTICOS"
                      icon={<FlaskConical className="h-5 w-5 text-[#2563EB]" />}
                    >
                    ...
                    <Card title="DISPOSITIVOS" icon={<Shield className="h-5 w-5 text-[#16A34A]" />}>
                      <div className="p-4 bg-[#F0FDF4] border border-[#BBF7D0] rounded-2xl">
                        <p className="text-xs font-black text-[#166534] uppercase tracking-tight">
                  ...
                  <Card
                    title="LABORATÓRIO IMPORTANTE"
                    icon={<Info className="h-5 w-5 text-[#2563EB]" />}
                  >
                    <div className="bg-[#0F172A] rounded-2xl p-6 shadow-inner">
                      <p className="text-xs font-mono leading-relaxed text-white opacity-90 uppercase">
                        {patient.laboratorio}
                      </p>
                    </div>
                  </Card>
                ...
                <div className="space-y-8">
                  <div className="bg-white rounded-[2.5rem] p-8 border border-[#DCE4F0] shadow-sm">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#0F172A]">
                        PENDÊNCIAS CRÍTICAS
                      </h3>
                    </div>
                    <p className="text-xs font-bold leading-relaxed text-[#0B1F44] uppercase">
                      {patient.pendencias}
                    </p>
                  </div>

                  <Card title="INTERCORRÊNCIAS" icon={<Clock className="h-5 w-5 text-[#8A9BB8]" />}>
                    <p className="text-xs font-bold leading-relaxed text-[#64748B] uppercase">
                      {patient.intercorrencias}
                    </p>
                  </Card>
                  ...
                  <div className="rounded-[2.5rem] border-2 border-[#FCA5A5] bg-[#FEF2F2] p-8">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#EF4444]">
                        ALERTAS DO ROUND
                      </h3>
                    </div>
                    <div className="space-y-4">
                      {patient.alertas.map((a, i) => (
                        <div key={i} className="flex items-start gap-3">
                          ...
                          <p className="text-xs font-black text-[#B91C1C] uppercase leading-tight">
                            {a}
                          </p>
                        </div>
                      ))}
      ...
      {showImport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 no-print">
          <div
            ...
            onClick={() => !isProcessing && setShowImport(false)}
          />
            ...
            <header className="px-10 py-8 border-b border-[#DCE4F0] flex items-center justify-between bg-[#F8FAFC]">
              <div>
                <h3 className="text-xl font-black text-[#0F172A] uppercase tracking-tight">
                  Importar Evoluções de Ontem
                </h3>
                <p className="text-[10px] font-black text-[#8A9BB8] uppercase tracking-[0.2em] mt-1">
                  MOTOR LUAN — INTELIGÊNCIA CLÍNICA
                </p>
              </div>
              <button
                onClick={() => setShowImport(false)}
                className="h-12 w-12 rounded-2xl border border-[#DCE4F0] flex items-center justify-center text-[#64748B] hover:bg-white transition-all"
              >
                <X className="h-6 w-6" />
              </button>
            </header>
              ...
              {isProcessing ? (
                ...
                <div className="p-10 space-y-8">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm font-black text-[#0F172A] uppercase tracking-widest">
                      {importedPatients.length} Pacientes Identificados
                    </h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {importedPatients.map((p) => (
                      <div
                        key={p.id}
                        className="p-5 rounded-2xl border border-[#DCE4F0] bg-[#F8FAFC] hover:border-[#7C3AED]/30 transition-all"
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-black text-[#0F172A]">
                            {p.leito} — {p.nome}
                          </span>
                          <button className="text-[#8A9BB8] hover:text-[#EF4444]">
                            <X className="h-4 w-4" />
                          </button>
                        </div>
                        <p className="text-[10px] font-bold text-[#64748B] uppercase truncate">
                          {p.diagnosticos}
                        </p>
                        <div className="flex flex-wrap gap-1 mt-3">
                          {p.alertas.map((a) => (
                            <span
                              key={a}
                              className="text-[8px] font-black uppercase px-2 py-0.5 rounded-md bg-white border border-[#DCE4F0] text-[#EF4444]"
                            >
                              {a}
                            </span>
                          ))}
                ...
                <div className="p-10">
                  <div className="flex gap-4 mb-8">
                    {["COLAR TEXTO", "ENVIAR ARQUIVOS", "FOTO / PRINT"].map((t, i) => (
                      ...
                      >
                        {i === 0 ? (
                          <ClipboardPaste className="h-4 w-4 mx-auto mb-2" />
                        ) : i === 1 ? (
                          <Upload className="h-4 w-4 mx-auto mb-2" />
                        ) : (
                          <ImageIcon className="h-4 w-4 mx-auto mb-2" />
                        )}
                        {t}
                      </button>
                    ))}
              ...
              {!isReviewing ? (
                ...
                <>
                  <button
                    ...
                    className="px-8 py-4 rounded-2xl border-2 border-[#DCE4F0] text-[#0F172A] font-black uppercase tracking-widest text-[11px]"
                  >
                    VOLTAR
                  </button>
                  <button
                    onClick={confirmImport}
                    className="px-10 py-4 rounded-2xl bg-[#2563EB] text-white font-black uppercase tracking-widest text-[11px] shadow-xl shadow-blue-500/20"
                  >
                    USAR PARA GERAR MAPA
                  </button>
                </>
              )}
            </footer>
          ...
          <div className="relative w-full max-w-5xl bg-white rounded-[3rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-300">
            <header className="px-10 py-8 border-b border-[#DCE4F0] flex items-center justify-between bg-[#F8FAFC]">
              <div>
                ...
                <p className="text-[10px] font-black text-[#8A9BB8] uppercase tracking-[0.2em] mt-1">
                  CLÍNICA MÉDICA FEMININA (CMF) — {TODAY}
                </p>
              </div>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => window.print()}
                  className="h-12 px-6 rounded-2xl bg-[#2563EB] text-white text-[10px] font-black uppercase tracking-widest hover:bg-[#1D4ED8] flex items-center gap-2 shadow-lg shadow-blue-500/20 transition-all"
                >
            ...
            <footer className="px-10 py-8 border-t border-[#DCE4F0] bg-white flex justify-end gap-3">
              ...
              <button
                onClick={() => setShowMapa(false)}
                className="px-10 py-4 rounded-2xl bg-[#0F172A] text-white font-black uppercase tracking-widest text-[11px]"
              >
                FECHAR
              </button>
            </footer>
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <header className="px-10 py-8 border-b border-[#DCE4F0] flex items-center justify-between bg-[#F8FAFC]">
              <div>
                ...
                <p className="text-[10px] font-black text-[#8A9BB8] uppercase tracking-[0.2em] mt-1">
                  ROTEIRO PARA ROUND COLETIVO
                </p>
              </div>
              <button
                onClick={() => setShowBriefing(false)}
                className="h-10 w-10 rounded-2xl border border-[#DCE4F0] flex items-center justify-center text-[#64748B]"
              >
                <X className="h-5 w-5" />
              </button>
            </header>
            ...
            <footer className="px-10 py-8 border-t border-[#DCE4F0] flex justify-end gap-3">
              <button
                ...
                className="px-8 py-4 rounded-2xl border-2 border-[#DCE4F0] text-[#0F172A] font-black uppercase tracking-widest text-[11px] flex items-center gap-2"
              >
                <Copy className="h-4 w-4" /> COPIAR BRIEFING
              </button>
              <button
                onClick={() => setShowBriefing(false)}
                className="px-8 py-4 rounded-2xl bg-[#0F172A] text-white font-black uppercase tracking-widest text-[11px]"
              >
                FECHAR
              </button>
            </footer>
      ...
      <div className="print-only p-12">
        ...
        <div className="space-y-8">
          {patients.map((p) => (
            <div
              key={p.id}
              className="pb-8 border-b-2 border-dashed border-[#DCE4F0] last:border-0 page-break-inside-avoid"
            >
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-black text-[#0F172A] uppercase tracking-tight">
                  {p.leito} — {p.nome} ({p.idade}A)
                </h2>
              </div>
              <div className="grid grid-cols-1 gap-3 text-[11px] font-bold uppercase leading-relaxed text-[#0F172A]">
                <p>
                  <span className="text-[#8A9BB8]">DIAGNÓSTICOS:</span> {p.diagnosticos}
                </p>
                <p>
                  <span className="text-[#8A9BB8]">ATB:</span> {p.antibioticos}
                </p>
                <p>
                  <span className="text-[#8A9BB8]">QUADRO:</span> {p.quadro}
                </p>
                <p>
                  <span className="text-[#8A9BB8]">PENDÊNCIAS:</span> {p.pendencias}
                </p>
                {p.alertas.length > 0 && (
                  <p className="text-[#EF4444]">
                    <span className="text-[#8A9BB8]">ALERTAS:</span> {p.alertas.join("; ")}
                  </p>
                )}
              </div>
            </div>
        ...
        <footer className="mt-12 pt-8 border-t border-[#DCE4F0] text-center">
          <p className="text-[9px] font-black text-[#CBD5E1] uppercase tracking-[0.5em]">
            GERADO PELO SISTEMA MEDFLUXO — MOTOR LUAN IA
          </p>
        </footer>
      </div>
    </div>
...
function Card({
  ...
  return (
    <div
      className={`rounded-[2.5rem] border p-8 shadow-sm transition-all duration-300 hover:shadow-md ${accent ? "bg-[#F0F7FF] border-[#BFDBFE]" : "bg-white border-[#DCE4F0]"}`}
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div
            className={`h-10 w-10 rounded-xl flex items-center justify-center ${accent ? "bg-white text-[#2563EB]" : "bg-[#F8FAFC] text-[#64748B]"}`}
          >
            {icon}
          </div>
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#0F172A]">
        ...
        {badge && (
          <span className="text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-full bg-[#F3E8FF] text-[#7C3AED] border border-[#7C3AED]/10">
            {badge}
          </span>
        )}
      </div>
