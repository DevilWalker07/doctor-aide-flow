        ...
        <div className="bg-white border border-border rounded-[3rem] p-10 shadow-xl">
              ...
              <div className="space-y-3">
                {paciente.pendencias && paciente.pendencias.length > 0 ? (
                  paciente.pendencias.map((p: any) => (
                    <div
                      key={p.id}
                      className="flex gap-3 items-center bg-amber-500/5 p-4 rounded-xl border border-amber-500/10 text-xs font-bold text-amber-700 uppercase tracking-tight"
                    >
          ...
          <div className="mt-12 p-8 bg-primary/5 rounded-[2.5rem] border border-primary/10 text-center">
            <p className="text-sm font-bold text-primary uppercase tracking-[0.2em] mb-4">
              Módulo de Persistência (Fase 3)
            </p>
            <p className="text-xs text-muted-foreground max-w-md mx-auto leading-relaxed">
              Na Fase 3, este paciente será persistido no Supabase e estará disponível em todo o
              hospital.
            </p>
            <button
