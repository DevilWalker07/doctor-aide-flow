...
export function OrientacoesA4({ doc, medico }: { doc: OrientacoesDocumento; medico: MedicoDocumento }) {
  const orientacoes = doc.orientacaoIds.map(getOrientacao).filter((o): o is NonNullable<typeof o> => Boolean(o));
  const vazio = orientacoes.length === 0 && doc.extras.length === 0;

  return (
    <DocumentoA4 medico={medico} paciente={doc.paciente} titulo="Orientações ao paciente" data={doc.data} testid="doc-preview-orientacoes">
      {vazio && <p className="text-[10pt] italic text-black/50 py-6 text-center">Selecione temas ou escreva orientações para montar o documento.</p>}

      <div className="space-y-4">
        {orientacoes.map((o) => (
          <section key={o.id} className="a4-row">
            <h3 className="text-[11pt] font-black uppercase tracking-wide border-b border-black/30 mb-2">{o.titulo}</h3>
        ...
        {doc.extras.length > 0 && (
          <section className="a4-row">
            <h3 className="text-[11pt] font-black uppercase tracking-wide border-b border-black/30 mb-2">Outras orientações</h3>
            <ul className="space-y-1.5 text-[10.5pt] leading-snug">
              {doc.extras.map((e, i) => (
                <li key={i} className="flex gap-2">
                  <CheckCircle2 className="h-4 w-4 shrink-0 mt-0.5" aria-hidden="true" />
