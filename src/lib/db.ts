...
export async function mergePatientData(
  ...
  if (newData.antibiotics?.length) {
    const existingNames = new Set(
      (existing.antibiotics ?? []).map((a) => a.nome?.trim().toLowerCase()),
    );
  ...
  if (newData.conducts?.length) {
    const existingTexts = new Set(
      (existing.conducts ?? []).map((c) => (typeof c === "string" ? c.trim().toLowerCase() : "")),
    );
...
export async function getLastEvolution(
  patientId: string,
  userId: string,
...
export async function getOutpatientDocumentsByPatient(
  patientId: string,
  userId: string,
): Promise<OutpatientDocumentRow[]> {
  const { data, error } = await supabase
