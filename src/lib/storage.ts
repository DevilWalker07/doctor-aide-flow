export const storage = {
  ...
  setAtbAlertDays: (days: number) => localStorage.setItem("da_atb_alert_days", String(days)),
  ...
  mergeLocalPatient: (id: string, newData: any) => {
    ...
    if (newData.problem_list?.length) {
      const existingTexts = new Set(
        (existing.problem_list ?? []).map((p: any) => String(p).trim().toLowerCase()),
      );
      const newProblems = newData.problem_list.filter(
        (p: string) => p && !existingTexts.has(p.trim().toLowerCase()),
      );
    ...
    if (newData.antibiotics?.length) {
      const existingNames = new Set(
        (existing.antibiotics ?? []).map((a: any) => a.nome?.trim().toLowerCase()),
      );
      const newAtbs = newData.antibiotics.filter(
        (a: any) => a.nome && !existingNames.has(a.nome.trim().toLowerCase()),
      );
    ...
    if (newData.medications?.length) {
      const existingTexts = new Set(
        (existing.medications ?? []).map((m: any) => String(m).trim().toLowerCase()),
      );
      const newMeds = newData.medications.filter(
        (m: string) => m && !existingTexts.has(m.trim().toLowerCase()),
      );
    ...
    if (newData.conducts?.length) {
      const existingTexts = new Set(
        (existing.conducts ?? []).map((c: any) => String(c).trim().toLowerCase()),
      );
      const newConducts = newData.conducts.filter(
        (c: string) => c && !existingTexts.has(c.trim().toLowerCase()),
      );
    ...
    if (newData.pending_issues?.length) {
      const existingTexts = new Set(
        (existing.pending_issues ?? []).map((p: any) => String(p).trim().toLowerCase()),
      );
