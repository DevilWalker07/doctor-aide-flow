...
const PACIENTE_VAZIO: PacienteDocumento = {
  pacienteId: null,
  nome: "",
  idade: "",
  sexo: "",
...
export function useDocumentoBase(pacienteId: string | undefined) {
  ...
  useEffect(() => {
    if (vinculado) {
      setPacienteForm({
        pacienteId: vinculado.pacienteId,
        nome: vinculado.nome,
        idade: vinculado.idade,
        sexo: vinculado.sexo,
        documento: "",
        leito: vinculado.leito,
      });
    }
  }, [vinculado]);
  ...
  const salvar = async (doc: OutpatientDocument) => {
    ...
    try {
      ...
      if (result.origem === "supabase") toast.success("Documento salvo no banco.");
        ...
        toast.info(
          result.erro
            ? `Salvo apenas neste dispositivo (banco indisponível: ${result.erro}).`
