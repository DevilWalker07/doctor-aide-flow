...
export const LAB_THRESHOLDS: Record<LabKey, Threshold> = {
  ...
  LEUCO: {
    label: "Leucócitos",
    unit: "/mm³",
    aliases: ["LEUCO", "LEUCOCITOS", "LEUCOS", "WBC", "GB"],
    critLow: 1000,
    warnLow: 4000,
    warnHigh: 12000,
    critHigh: 30000,
  ...
  PLAQ: {
    label: "Plaquetas",
    unit: "/mm³",
    aliases: ["PLAQ", "PLAQUETAS", "PLQ", "PLT"],
    critLow: 50000,
    warnLow: 100000,
  },
  CR: {
  ...
  GLI: {
    label: "Glicemia",
    unit: "mg/dL",
    aliases: ["GLI", "GLICEMIA", "GLICOSE", "HGT", "DX", "DEXTRO"],
    critLow: 60,
    warnLow: 70,
    warnHigh: 180,
    critHigh: 350,
...
const ALIAS_INDEX: Record<string, LabKey> = Object.fromEntries(
  (Object.keys(LAB_THRESHOLDS) as LabKey[]).flatMap((key) =>
    LAB_THRESHOLDS[key].aliases.map((a) => [a, key]),
  ),
...
export function classifyLab(
  key: LabKey,
  value: number,
): { severity: LabSeverity; direction: "low" | "high" | null } {
...
const fmt = (n: number) =>
  Number.isInteger(n) ? n.toLocaleString("pt-BR") : n.toFixed(1).replace(".", ",");
